...
def test_transform_lateral_flatten() -> None:
    ...
    expected = strip(
        "SELECT * FROM _FS_FLATTEN_ARRAY([1, 2], '', FALSE, FALSE, 'BOTH') AS F(SEQ, KEY, PATH, INDEX, VALUE, THIS)"
    )
...
def test_transform_table_flatten() -> None:
    ...
    expected = strip("SELECT * FROM _FS_FLATTEN_ARRAY([1, 2], '', FALSE, FALSE, 'BOTH') AS F")
...
def test_flatten_alias_none(cur: snowflake.connector.cursor.SnowflakeCursor) -> None:
    ...
    assert sqlglot.parse_one(
        ...
        "SELECT * FROM _FS_FLATTEN_ARRAY([1, 2], '', FALSE, FALSE, 'BOTH')"
    )
    cur.execute(sql)
...
def test_flatten_alias_rename(cur: snowflake.connector.cursor.SnowflakeCursor) -> None:
    ...
    assert sqlglot.parse_one(
        ...
        "SELECT * FROM _FS_FLATTEN_ARRAY([1, 2], '', FALSE, FALSE, 'BOTH') AS rename(a, b, c)"
    )
    cur.execute(sql)
...
def test_flatten_recursive(cur: snowflake.connector.cursor.SnowflakeCursor):
    cur.execute(
        """
        SELECT key, path, index
        FROM TABLE(FLATTEN(
            input => PARSE_JSON('{"a":[1,{"b":2}],"c":3}'),
            recursive => TRUE
        ))
        """
    )
